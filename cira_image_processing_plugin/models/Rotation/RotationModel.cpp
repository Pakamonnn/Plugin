...
void RotationModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void RotationModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void RotationModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &RotationModel::runProcess, _flowDataIn);
