...
void BlurModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void BlurModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void BlurModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &BlurModel::runProcess, _flowDataIn);
