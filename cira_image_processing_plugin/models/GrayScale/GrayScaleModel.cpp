...
void GrayScaleModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void GrayScaleModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void GrayScaleModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &GrayScaleModel::runProcess, _flowDataIn);
