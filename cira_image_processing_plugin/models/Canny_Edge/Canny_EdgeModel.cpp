...
void Canny_EdgeModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void Canny_EdgeModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void Canny_EdgeModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &Canny_EdgeModel::runProcess, _flowDataIn);
