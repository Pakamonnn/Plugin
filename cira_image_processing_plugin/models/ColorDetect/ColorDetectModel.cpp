...
void ColorDetectModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
  ...
  if(form->workerThread->isUseImage) {
    flowdata._img = form->workerThread->mat_mask;
  }
...
void ColorDetectModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void ColorDetectModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &ColorDetectModel::runProcess, _flowDataIn);
