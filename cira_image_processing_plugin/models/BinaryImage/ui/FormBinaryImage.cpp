#include "FormBinaryImage.h"
#include "ui_FormBinaryImage.h"

FormBinaryImage::FormBinaryImage(QWidget *parent) :
  QWidget(parent),
  ui(new Ui::FormBinaryImage)
{
  ui->setupUi(this);
  ...
  workerThread = new ThreadBinaryImage();
  dialog = new DialogBinaryImage();

  mv_node_run = new QMovie(":/cira_image_processing_plugin/icon/run_led.gif");
  update_ui();
  connect(GlobalData::GlobalDataObject, SIGNAL(stopAllScene()), this, SLOT(stop_node_process()));
}

FormBinaryImage::~FormBinaryImage()
{
  delete ui;
}

void FormBinaryImage::on_pushButton_nodeEnable_clicked()
{
...
void FormBinaryImage::on_pushButton_prop_clicked()
{
