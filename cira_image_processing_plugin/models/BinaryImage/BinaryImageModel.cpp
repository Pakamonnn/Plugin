...
void BinaryImageModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void BinaryImageModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void BinaryImageModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &BinaryImageModel::runProcess, _flowDataIn);
