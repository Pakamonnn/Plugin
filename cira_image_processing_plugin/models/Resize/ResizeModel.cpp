...
void ResizeModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void ResizeModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void ResizeModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &ResizeModel::runProcess, _flowDataIn);
