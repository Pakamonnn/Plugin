...
void MorphologyModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void MorphologyModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void MorphologyModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &MorphologyModel::runProcess, _flowDataIn);
