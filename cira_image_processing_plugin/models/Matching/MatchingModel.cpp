...
void MatchingModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void MatchingModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void MatchingModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &MatchingModel::runProcess, _flowDataIn);
