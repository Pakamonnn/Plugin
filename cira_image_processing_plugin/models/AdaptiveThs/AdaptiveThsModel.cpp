...
void AdaptiveThsModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void AdaptiveThsModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void AdaptiveThsModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &AdaptiveThsModel::runProcess, _flowDataIn);
