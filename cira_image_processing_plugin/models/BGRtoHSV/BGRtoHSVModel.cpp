...
void BGRtoHSVModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void BGRtoHSVModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void BGRtoHSVModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &BGRtoHSVModel::runProcess, _flowDataIn);
