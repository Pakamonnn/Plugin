...
void Find_EdgeModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void Find_EdgeModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void Find_EdgeModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &Find_EdgeModel::runProcess, _flowDataIn);
