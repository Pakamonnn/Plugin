...
void FlipModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void FlipModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void FlipModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &FlipModel::runProcess, _flowDataIn);
