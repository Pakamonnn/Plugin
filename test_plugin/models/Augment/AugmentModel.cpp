...
void AugmentModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
  ...
  if(form->workerThread->isUseImage) {
    flowdata._img = form->workerThread->image_out; //.........................
  }
...
void AugmentModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void AugmentModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &AugmentModel::runProcess, _flowDataIn);
