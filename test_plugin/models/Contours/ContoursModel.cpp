...
ContoursModel()
{
  ...
  //*****************************
  _flowDataOut2.reset();
  _flowDataOut2 = std::make_shared<FlowData>(flowdata_);
  emit dataUpdated(1);

}

void ContoursModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
  ...
  if(form->workerThread->isUseImage) {
    flowdata._img = form->workerThread->mat_out;
  ...
  //*******
  flowdata._img = form->workerThread->mat_out2;
  _flowDataOut2 = std::make_shared<FlowData>(flowdata);
  form->nodeStatus_complete = true;
  emit dataUpdated(1);
...
void ContoursModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void ContoursModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &ContoursModel::runProcess, _flowDataIn);
