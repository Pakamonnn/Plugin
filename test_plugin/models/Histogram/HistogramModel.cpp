...
void HistogramModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
  ...
  //**********************
  flowdata._img = form->workerThread->mat_im2;
...
void HistogramModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void HistogramModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &HistogramModel::runProcess, _flowDataIn);
