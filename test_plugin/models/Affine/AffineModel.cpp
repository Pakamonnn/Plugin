...
void AffineModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
  ...
  if(form->workerThread->isUseImage) {
    ...
    //set image to tmp_img
    form->dialog->mtx.lock();
    form->dialog->tmp_im = form->workerThread->mat_im.clone();
...
void AffineModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void AffineModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &AffineModel::runProcess, _flowDataIn);
